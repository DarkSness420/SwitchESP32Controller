...
#include "ssd1306.h"
SSD1306_t oled;
#define SDA_PIN    21   // GPIO number connected to OLED SDA
#define SCL_PIN    22   // GPIO number connected to OLED SCL
#define RESET_PIN  -1   // GPIO connected to OLED reset (or -1 if none)
...
void oled_display_text_multiline(SSD1306_t *oled, uint8_t start_page, const char *text) {
    ...
    while (*p && page < ssd1306_get_pages(oled)) {
        if (*p == '\n') {
            size_t len = p - line_start;
            ssd1306_display_text(oled, page, line_start, len, false);
            page++;
            p++;
            line_start = p;
        } else {
    ...
    // Print last line if any left
    if (line_start != p && page < ssd1306_get_pages(oled)) {
        size_t len = p - line_start;
        ssd1306_display_text(oled, page, line_start, len, false);
    }
...
static void uart_task() {
  ...
  while (1) {
    ...
    if (len > 0) {
      ...
      if (len > 9) {
        int str_len = len - 9;  // length from the 10th byte to the end
        // Make sure the buffer size is enough for the string + null terminator
        if (str_len >= BUF_SIZE) {
            str_len = BUF_SIZE - 1;
        }
        char text_buf[BUF_SIZE];
        memcpy(text_buf, &uart_data[9], str_len);  // copy from 10th byte
        text_buf[str_len] = '\0';                   // null terminate
        oled_display_text_multiline(&oled, 1, text_buf);
      }
      if (len == 9) {
...
void oled_display_text_line(SSD1306_t *oled, uint8_t page, const char *text) {
    ssd1306_clear_screen(oled, false);
    ssd1306_display_text(oled, page, text, strlen(text), false);
    ssd1306_show_buffer(oled);
}
...
void app_main() {
    i2c_master_init(&oled, SDA_PIN, SCL_PIN, RESET_PIN); // or spi_master_init(...)
    ssd1306_init(&oled, 128, 64);
    oled_display_text_multiline(&oled, 1, "Started!");
